...
const TruthLog = ({ entries }: TruthLogProps) => {
  ...
  return (
    <div className="absolute right-4 top-32 bottom-4 w-80 md:w-96 z-20">
      <div className="glass h-full flex flex-col">
        {/* Header */}
        <div className="px-4 py-3 border-b border-primary/30">
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-full bg-primary animate-pulse" />
            <span className="text-xs tracking-[0.2em] text-primary">GEMINI ANALYSIS LOG</span>
          </div>
        </div>
        ...
        <div 
          ref={scrollRef}
          className="flex-1 overflow-y-auto p-4 space-y-2 scrollbar-thin scrollbar-thumb-primary/30 scrollbar-track-transparent"
        >
          {entries.length === 0 ? (
            ...
            entries.map((entry) => (
              <div key={entry.id} className="text-xs font-mono leading-relaxed">
                ...
                <span className={`ml-2 ${getTypeStyles(entry.type)}`}>
                  {getTypePrefix(entry.type)}
                </span>
                <span className="ml-2 text-foreground/90">{entry.message}</span>
                {entry.value !== undefined && (
                  <span className="ml-2 text-hud-price text-glow">${entry.value}</span>
                )}
              </div>
            ))
          )}
        ...
        {/* Footer */}
        <div className="px-4 py-2 border-t border-primary/30 text-xs text-muted-foreground">
